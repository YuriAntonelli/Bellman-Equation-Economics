...
#--------------------------------------#
#-STEP 5: Horward improvement function-#
#--------------------------------------#
...
V_ast3, Gamma_ast3 = valueFunctionIteration(e, h)
...
"""
# Grids
e = np.linspace(1, 500, 50) 
for i in range(len(e)): 
    e[i] = round(e[i])
    
h = np.linspace(1, 70, 50)  
for i in range(len(h)): 
    h[i] = round(h[i])
...
def computeMax(e_index, h_index, e, h, V):
    ...
    for i in range(N_e):
        for j in range(N_h):
            value = objValue(e_index, h_index, i, j, e, h, V)
            if value > solution:
                solution = value
                optindex = [i, j]
    return solution, optindex 
    ...
    while dist > epsilon:
        ...
        for i in range(N_e):
            for j in range(N_h):
                N += 1
                inext, jnext = gamma[N]
                HV[i, j] = objValue(i, j, inext, jnext, e, h, V) 
        dist = (abs(HV-V)).max()
        V = HV.copy()
    return HV 
...
def valueFunctionIteration(e, h):
    ...
    while dist > epsilon:
        TV, gamma = newValueFunction(e, h, V)
        Vnext = howardImprovement(gamma, e, h, TV)
        dist = (abs(V-Vnext)).max()
        V = Vnext.copy()
        loops += 1 
        print(f'loop number {loops} with distance {dist}')
